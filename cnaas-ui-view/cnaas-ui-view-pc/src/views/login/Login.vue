...
export default {
  ...
  methods: {
    handleLogin() {
      ...
      //   .then((res) => {
      //     // 保存token认证信息
      //     this.$ls.set('Authorization', res);
      //     resolveResponse(this, getLoginUser())
      //       // 若更换用户则将页签清零
      //       .then((newUserInfo) => {
      //         const oldUserInfo = this.$ls.get('userInfo');
      //         if (oldUserInfo === undefined
      //           || oldUserInfo === null
      //           || oldUserInfo === ''
      //           || oldUserInfo.account !== newUserInfo.account) {
      //           // 获取当前用户信息并存储（账号、权限、电话等）
      //           this.$ls.set('userInfo', newUserInfo);
